...
<input type='button' class='add-btn' onclick=location.href='" . JURI::base() . "system-management-vic/template-listing-vic/template-management?view=add' value='Add New'>
</div>";
...
$sql = "
	...
	ORDER BY document_handler_entity.name, document_handler_folder.name, document_handler_file.name 
";
...
$loop = mysql_query($sql)
	...
	echo "<table class='listing-table table-bordered'><thead><tr><th>Sections</th><th>Categories</th><th>Templates</th> </tr></thead><tbody>";
while ($record = mysql_fetch_assoc($loop))
    echo "<tr class='pointer' onclick=location.href='" . $this->baseurl . "template-listing-vic/template-manage-vic?module=quote&document_handler_entity_id={$record['document_handler_entity_id']}&document_handler_folder_id={$record['document_handler_folder_id']}&document_handler_file_id={$record['document_handler_file_id']}' ><td>{$record['document_handler_entity_name']}</td><td>{$record['document_handler_folder_name']}</td><td>{$record['document_handler_file_name']}</td></tr>";
    echo "</tbody></table>"; 
