...
if(isset($user->groups['10'])){
  ...
  //error_log("_REQUEST: ".print_r($_REQUEST,true), 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
  $cf_id = "0";
  ...
  //error_log("2: ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
  }
  ...
  error_log("Status: 1". $Status, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
...
if(isset($_FILES['signed_doc'])){  //Upload file from Files tab
  ...
  mysql_query($query) or trigger_error("Insert failed: " . mysql_error());
    ...
    $query = "UPDATE  ver_chronoforms_data_letters_vic SET has_upload_file=1 WHERE cf_id={$doc_id} ";
  //$query = "UPDATE  ver_chronoforms_data_letters_vic SET is_temp=1 WHERE cf_id={$doc_id} ";
      mysql_query($query) or trigger_error("Insert failed: " . mysql_error());
      header('Location:'.JURI::base().'client-listing-vic/client-folder-vic?pid='.$id);
 }  
...
if(isset($_POST['delete-drawing'])) {
  ...
  <input type='hidden' name='user_group' id='user_group' value="system_admin" / >
  ...
  <div id="quote" class="tab_content" style="display: block;">
    ...
    ?>
        <input type="button" value="Add New Costing" <?php echo "onclick=location.href='" . JURI::base() . "add-quote-vic?quoteid=". $QuoteID."&ref={$ref}&uc=" . date('YmdHisu') . "'"; ?>  />
    <?php } //end if?>
  ...
  {  
    ...
    if($info['has_upload_file']==1){
      ...
      { 
       ...
       echo "<li><a href=\"".$row['photo']."\" download class='remove-link'>   ".$row['file_name']."</a>    <span class=\"ui-icon ui-icon-closethick\" style='display:inline-block; cursor: pointer;' onclick=\"if(confirm('Are you sure you want to delete?')){"."$('#picid').val('".$row["cf_id"]."'); $('#btn_picid').click();}\"   > </span></li>";
      }
      echo "</ul>
      </div>";
    ...
    <div class="drawing-tbl">
      ...
      if($info['has_upload_file']==1){
        echo '<div> 
          ...
          $resultimg = mysql_query("SELECT cf_id, clientid, photo, file_name FROM ver_chronoforms_data_pics_vic WHERE clientid = '$ClientID'  AND upload_type ='signed_correspondence_doc' AND ref_id={$info['cf_id']} ");
          $thumbnail = "";
          while($row = mysql_fetch_array($resultimg))
          { 
          if(strtolower(substr($row['photo'],-3))=="pdf"){
...
<div class="drawing-tbl">
  ...
  <div id="_tabs_wrapper" class="button-tab">
    ...
    <?php }else{ ?> 
        ...
        ?>
            <input type="submit" value="Delete" id="bsbtn" name="delete" class="bbtn" onclick="return confirm('Are you sure you want to delete quote?');" >
        <?php } //end if?>
    ...
    <?php
    /* ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- */
    /* document system poc > demo button area                      */
    /* begin                                                       */
    /* ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- */
    ...
    ?>
        ...
        <input type="button" class="bbtn" id="button_document_system_poc_demo_button" name="button_document_system_poc_demo_button" value="Document System POC Demo" onclick="OpenDocumentSystemPocDemoWindow()">
    <?php
    }
    /* ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- */
    /* document system poc > demo button area                      */
