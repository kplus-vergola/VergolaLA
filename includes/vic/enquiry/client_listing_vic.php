...
//error_log("is_builder:".$is_builder, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
// $url_filter = "";
// if($is_builder){
...
//error_log(" total_records: ".$total_records, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
...
if(isset($_POST['download_pdf'])==true){
	...
	$html_pdf .= "<table border=\"1\" cellpadding=\"2\" BORDERCOLOR=\"GREY\"  > <tr><th width=\"60\" {$style}>&nbsp;<b>ID</b></th><th width=\"120\" {$style}>&nbsp;<b>Customer Name</b></th><th width=\"150\" {$style}>&nbsp;<b>Site Address</b></th><th width=\"80\" {$style}>&nbsp;<b>Home Phone</b></th><th width=\"90\" {$style}>&nbsp;<b>Mobile</b></th> <th width=\"120\" {$style}>&nbsp;<b>Consultant</b></th><th width=\"80\" {$style}>&nbsp;<b>Enquiry</b></th><th width=\"80\" {$style}>&nbsp;<b>Appointment</b></th><th width=\"80\" {$style}>&nbsp;<b>Quote Value</b></th><th width=\"80\" {$style}>&nbsp;<b>Delivered</b></th><th width=\"80\" {$style}>&nbsp;<b>Follow Up</b></th><th width=\"70\" {$style}>&nbsp;<b>Status</b></th><th width=\"350\" {$style} >&nbsp;<b>Note</b> </th> </tr>";
} 
...
while ($record = mysql_fetch_assoc($loop)) {
	...
	$consultant_name_css = (isset($user->groups['9'])) ? "hide" : "";
	$consultant_name = (isset($_POST['download_pdf']) ? addslashes($record['repname']) : $record['repname']);
	$lead_type = (isset($_POST['download_pdf']) ? addslashes($record['leadname']) : $record['leadname']);
	$enquiry_date = date(PHP_DFORMAT, strtotime($record['datelodged']));
	$appointment_date = (empty($record['appointmentdate']) == false && $record['appointmentdate'] != "" && $record['appointmentdate'] != "0000-00-00 00:00:00" && $record['appointmentdate'] != "1900-01-00 00:00:00") ? date(PHP_DFORMAT . ' @ h:i A', strtotime($record['appointmentdate'])) : '';
	$quote_value = ($record['total_cost'] > 0) ? "$" . number_format($record['total_cost'], 2, ".", ",") : "";
	$quote_delivered = (empty($record['qdelivered']) == false && $record['qdelivered'] != "" && $record['qdelivered'] != "0000-00-00") ? date(PHP_DFORMAT,strtotime($record['qdelivered'])) : '';
	if (empty($record['ffdate1'] == false && $record['ffdate1'] != "0000-00-00")) {
		$follow_up_date = date(PHP_DFORMAT,strtotime($record['ffdate1']));
	} elseif (empty($record['appointmentdate']) == false && $record['appointmentdate'] != "0000-00-00 00:00:00" && $record['appointmentdate'] != "1900-01-00 00:00:00") {
		$follow_up_date = date(PHP_DFORMAT.' @ h:i A',strtotime($record['appointmentdate']));
	} else {
		$follow_up_date = '';
	}
	...
	$replace_value_html_template_data_row_client_listing = array(
		$target_click_url_attr, 
		$td_style, 
		$client_id, 
		$client_header_css,
	...
	$html_data_rows_client_listing .= str_replace(
		$search_tag_html_template_data_row_client_listing, 
		$replace_value_html_template_data_row_client_listing, 
		$html_template_data_row_client_listing
	);
	...
	"<td {$style}>".(isset($_POST['download_pdf'])?addslashes($record['site_address1']):$record['site_address1'])."</td>"  .
	...
	$html .=  "<td {$style}>";
		if (empty($record['qdelivered'])==false && $record['qdelivered']!="" && $record['qdelivered']!="0000-00-00"){$html .=  date(PHP_DFORMAT,strtotime($record['qdelivered']));} else {$html .=  "";}
	$html .=  "</td><td {$style}>"; // Retrieve Follow Ups

	if(empty($record['ffdate1'])==false && $record['ffdate1']!="0000-00-00" ) {$html .=  date(PHP_DFORMAT,strtotime($record['ffdate1']));}
...
$html_table_client_listing = extractOuterStringFromText(
	'<!-- html_template_data_row_begin -->', 
	'header', 
	$html_template_table_client_listing 
);
...
$search_tag_html_table_client_listing = array(
	'<!-- html_template_data_row_section -->' 
);
$replace_value_html_table_client_listing = array(
	$html_data_rows_client_listing 
);
$html_table_client_listing = str_replace(
	$search_tag_html_table_client_listing, 
	$replace_value_html_table_client_listing, 
	$html_table_client_listing 
);
if (isset($_POST['download_pdf']) == true) {
	$html_pdf_client_listing = extractOuterStringFromText(
		'<!-- html_template_data_row_begin -->', 
		'header', 
	...
	$search_tag_html_pdf_client_listing = array(
		'<!-- html_template_data_row_section -->' 
	);
	$replace_value_html_pdf_client_listing = array(
		$html_data_rows_client_listing 
	);
	$html_pdf_client_listing = str_replace(
		$search_tag_html_pdf_client_listing, 
		$replace_value_html_pdf_client_listing, 
		$html_pdf_client_listing 
	);
}
    ...
    if(isset($_POST['download_pdf'])==true){
    	...
    	if($advance_search){
			...
			$html_pdf = "<div><b>Filter</b>  {$admin_add}   {$search_add}  &nbsp;&nbsp;  &nbsp;&nbsp;&nbsp;&nbsp; <br/> {$date_header} <br/> </div> <br/>" . $html_pdf_client_listing;
 
		}else{
			// $html_pdf = $html_pdf.$html;
			$html_pdf = $html_pdf_client_listing;
			$search_add = "";
			if(strlen($search_string)>0){
				$search_add = " <b>Search :&nbsp;&nbsp;</b>".$search_string;
		...
		$sql = "INSERT INTO ver_chronoforms_data_letters_vic (clientid, template_name, datecreated, template_content) 
			 VALUES ('{$user->id}','$title', NOW(), '{$html_pdf}')"; 
...
// $html = $html_head.$html;	
$html = $html_table_client_listing;
echo $html;  
...
pagination($page, $total_records, $paging_url);
echo "</div></div>";
//error_log("end php: ".microtime(true), 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
?>
