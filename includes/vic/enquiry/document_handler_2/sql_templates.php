...
$sql_template_retrieve_entity_search_list[] = "
    ...
    FROM ver_chronoforms_data_clientpersonal_vic AS cp 
    WHERE LOWER(cp.clientid) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_firstname) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_lastname) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_address1) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_address2) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_suburb) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_state) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_postcode) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_mobile) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_hmphone) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_wkphone) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.client_email) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(cp.datelodged) LIKE '%[ENTITY_SEARCH_WORD]%' 
...
$sql_template_retrieve_entity_search_list[] = "
    ...
    WHERE LOWER(dfu.quoteid) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(dfu.project_name) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(dfu.framework_type) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(dfu.sales_rep) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(dfu.quotedate) LIKE '%[ENTITY_SEARCH_WORD]%' 
    ORDER BY entity_source_display_name, entity_id 
    LIMIT 1000;
";
$sql_template_retrieve_entity_search_list[] = "
    ...
    WHERE LOWER(dcl.projectid) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(dcl.project_name) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(dcl.framework_type) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(dcl.framework) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(dcl.sales_rep) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(dcl.contractdate) LIKE '%[ENTITY_SEARCH_WORD]%' 
    ORDER BY entity_source_display_name, entity_id 
    LIMIT 1000;
";
...
$sql_template_retrieve_contact_search_list[] = "
    SELECT 
        'Contact' AS 'entity_source_display_name', 
        CONCAT(cp.client_firstname, ' ', cp.client_lastname, '(', cp.clientid, ')') AS 'entity_id', 
        CONCAT(
            'Contact > ', cp.clientid, ' > ', 
            'Name: ', cp.client_firstname, ' ', cp.client_lastname, '; ', 
            'Address: ', cp.client_address1, ', ', cp.client_address2, ', ', cp.client_suburb, ', ', cp.client_state, ', ', cp.client_postcode, '; ', 
...
$sql_template_retrieve_contact_search_list[] = "
    ...
    FROM ver_users 
        ...
        LEFT JOIN ver_usergroups 
            ON ver_user_usergroup_map.group_id = ver_usergroups.id 
    WHERE LOWER(ver_users.username) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(ver_users.first_name) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(ver_users.last_name) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(ver_usergroups.title) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(ver_users.mobile) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(ver_users.phone) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(ver_users.email) LIKE '%[ENTITY_SEARCH_WORD]%' 
    OR LOWER(ver_users.created_at) LIKE '%[ENTITY_SEARCH_WORD]%' 
    ORDER BY entity_source_display_name, entity_id 
    LIMIT 1000;
";
