...
$sql1 = "SELECT *,(SELECT content FROM ver_chronoforms_data_notes_vic WHERE clientid = b.builderid  ORDER BY cf_id DESC LIMIT 1) AS note FROM (SELECT * FROM ver_chronoforms_data_builderpersonal_vic AS b WHERE tenderid IS NOT NULL {$rep_filter} {$c_status_filter_a} AND tenderstatus='Yes' {$suburb_filter} {$date_filter} {$search_string_filter} ) AS b LEFT JOIN (SELECT * FROM (SELECT * FROM ver_chronoforms_data_followup_vic WHERE 1=1 {$rep_filter2} {$c_status_filter}   ORDER BY cf_id DESC) as f0  GROUP BY quoteid ) AS f ON f.quoteid=b.builderid
