...
<tr ><td colspan="8" class="subheading" data-section='Frame' > <?php if($cat['section']=="Frame"){ echo "Framework"; }else if($cat['section']=="Fixings"){ echo "Fittings"; }else{ echo $cat['section']; } ?></td></tr>
<tr>
...
// $sql = "SELECT *, i.rrp AS bom_rrp, ((bm.length_feet * 12) + bm.length_inch) AS length FROM ver_chronoforms_data_contract_bom_vic AS i LEFT JOIN ver_chronoforms_data_inventory_vic AS inv ON inv.inventoryid=i.inventoryid JOIN ver_chronoforms_data_contract_bom_meterial_vic AS bm ON bm.inventoryid = i.inventoryid WHERE  i.projectid = '$projectid' AND bm.projectid = '$projectid' AND inv.section='{$cat['section']}' AND i.is_reorder=0  {$order_by}  "; 

$sql = "SELECT *, i.rrp AS bom_rrp, ((i.length_feet * 12) + i.length_inch) AS length FROM ver_chronoforms_data_contract_bom_vic AS i LEFT JOIN ver_chronoforms_data_inventory_vic AS inv ON inv.inventoryid=i.inventoryid  WHERE  i.projectid = '$projectid' AND inv.section='{$cat['section']}' AND i.is_reorder=0  {$order_by}  "; 
...
?>  
	...
	<tr>
		 ...
		 // if($i==0) 
		 	echo "Inventory"; ?>
		 	<th> Qty </th><th> Length </th> <th> Fracs </th> <th> UOM </th> <th>Cost</th> <th >Supplier</th>  <th>Amount </th> 	
	...
	?>	 
			...
			<?php  
				...
				$m_qty = 1;  
				$m_qty = $m['qty'];
				$m_length = $m['bm_length'];// * floor($bm['length'] / $m['length_per_ea']); 
				if($m['uom']=="Inches"){
...
?>  
	...
	// $sql_1 = "SELECT bm.id, bm.contract_item_cf_id, bm.projectid, bm.inventoryid, bm.materialid, bm.raw_cost, bm.qty, bm.length, bm.supplierid,  m.raw_description, m.uom, m.is_per_length, m.length_per_ea, s.company_name, m.length_per_ea_us FROM  ver_chronoforms_data_contract_bom_meterial_vic AS bm  JOIN ver_chronoforms_data_materials_vic AS m ON m.cf_id=bm.materialid JOIN ver_chronoforms_data_supplier_vic AS s ON s.supplierid=m.supplierid  WHERE bm.projectid = '$ListProjectID' AND bm.inventoryid='{$bm['inventoryid']}' and bm.contract_item_cf_id={$bm['contract_item_cf_id']} GROUP BY bm.inventoryid, bm.materialid    "; 
