	...
	if($section=="frame"){
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			if($isFirst){
				...
				$itemName = listItem("louvres",$r);
				$lengthInput = "<input type=\"text\"    name=\"slength[]\" class=\"input-in\" value=\"{$r['length']}\"  />";
				if(METRIC_SYSTEM=="inch"){
					$lengthInput_ft = "<input type=\"text\"    class=\"input-ft\" value=\"".get_feet_value($r['length'])."\"    />";
				}	
			...
			$tr .= "<td class=\"td-qty\"><input type=\"text\" {$itemQty} class=\"qtylen ".($isFirst?"":($r["is_additional"] == 1?"louver-item-qty":""))."\" ".($isFirst?$itemQty:"")." name=\"qty[]\" value=\"{$r['qty']}\"></td>";  
