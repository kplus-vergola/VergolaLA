...
function generateRowItem($section,$projectId=null){
	...
	if($section=="frame"){
		...
		while ($r = mysql_fetch_assoc($fw)) { 
	 	...
	 	$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Single Bay VR1' AND LOWER(section) = 'flashings' AND inventoryid NOT IN ('IRV48') " ); // 
