...
$qResult = mysql_query("SELECT * FROM ver_chronoforms_data_clientpersonal_vic WHERE clientid  = '$QuoteID'");
...
$is_builder = 0;
//error_log(substr($QuoteID,3), 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
// if(substr($QuoteID,0,3)=="BRV"){
// 	$is_builder = 1;
// 	//error_log('is builder', 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
// } 
//error_log($QuoteID, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
?> 
...
$(document).ready(function() {
	...
	//$('#output').html('<?php //echo " "; ?>'); // update the DIV
	...
	//$('#framework :first').prop('selected', true);
   ...
   //          	});
   //          });
 	<?php //error_log($fw, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); ?>
		...
		//if ('Single Bay' == selectedValue || 'Single Bay - Drop-In' == selectedValue || 'Single Bay VR1' == selectedValue || 'Single Bay VR1 - Drop-In' == selectedValue) {
		 <?php if($fw=="VR1"){ ?>	
			$('#save-dropin').css({display: "none"});
			$('#save-close-dropin').css({display: "none"});
			$('#save-dropin-down').css({display: "none"});
		...
		<?php } else if($fw == "VR4"){ ?>	
			...
			$('#save-close-angle-down').css({display: "none"});
			$('#cancel-down').css({display: "inline-block"});
			if('Double Bay VR4' == selectedValue ){
 
				//$('#output').html('<?php //require "doublebay_vic_vr4.php"; ?>');
			 	
				$.when($.getScript('<?php echo JURI::base().'jscript/doublebay_vr2.js?v='.mt_rand(); ?>')).done(function() {
					console.log('done in vr5');
				});
...
<?php //error_log("QuoteID :".$QuoteID, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); ?>
...
<?php if($fw=="VR1"){ ?>
	<div id="singlebay_input" style="<?php if(METRIC_SYSTEM=="meter"){ echo "display:inline;";}else{echo "display:none;";} ?>">
		...
		<span id="width" class="quoteinfo  quote-length" style="display:none;"><label>Width</label><input type="text" value="" name="width" id="widthid" class="num invisible" /></span>
...
<div id="doubletable" style="<?php if(($fw=="VR2" || $fw=="VR3" || $fw=="VR3-G") && METRIC_SYSTEM=="meter"){ echo "display:inline-block;";}else{echo "display:none;";} ?>">
...
<div class="table-subtotal-holder" style="display:none">
	...
	<tbody>
		...
		<tr><td>Disbursement Sub Total </td><td><span style="margin-right:10px;">$</span><input type="text" class="" value="" id="total_disbursement" name="total_disbursement" readonly="readonly" /></td></tr>
		...
		<tr><td><?php if(HOST_SERVER=="LA"){echo "";}else{echo "GST";} ?>  </td><td><span style="margin-right:10px;">$</span><input type="text" class="" value="" id="total_gst" name="total_gst" /></td></tr>
...
<input type="hidden" id="ref" value="<?php echo $_REQUEST['ref'];?>" />
...
function listColours($selected=null,$item=null,$class=""){  
	...
	{ 
		...
		if ($selected != null && $c == $selected) { $r .= " selected=\"selected\"";} 
			...
			//if($e==0){$r .= " selected=\"selected\"";} else {$r .= "";} 
			$r .= "";
		}
		$r .= ">{$c}</option>";
		$e++;
...
<div> 
	<table style="float:left; margin-left:-9000px;" id="template_table">
		...
		<?php
			...
			$tr = "";
			$tr .= "<td class=\"td-item\">".listItem("louvers")." <input type=\"hidden\" class=\"price\" value=\"0\" category=\"misc\" />   </td>";
			$tr .= "<td><input type=\"button\" class=\"added_item\" value=\"Remove\" ></td>"; 
