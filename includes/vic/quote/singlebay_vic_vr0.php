	...
	if($section=="frame"){
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			if($isFirst){
				...
				// $lengthInput = "<input type=\"text\"  {$itemLen} name=\"slength[]\" class=\"width input-in louver-item-len\" value=\"0\" />";
				$lengthInput = "<input type=\"text\"  {$itemLen} name=\"slength[]\" class=\"width input-in\" value=\"0\" />";
				if(METRIC_SYSTEM=="inch"){
					// $lengthInput_ft = "<input type=\"text\"  {$itemLen_ft}   class=\"width input-ft louver-item-len\" value=\"0\" />";
			...
			$tr .= "<td class=\"td-qty\"><input type=\"text\" {$itemQty} class=\"{$className} ".($isFirst?"":"")."\" name=\"qty[]\" value=\"\"></td>";  
