...
//error_log($sql, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');    
    ...
    {   
	  ...
	  echo "<tr><td class=\"tbl-name\">".$rowff["project_name"]."</td>".
		   ...
		   onclick=location.href='" . JURI::base() . "client-listing-vic/client-folder-vic?pid={$id}&cf_id={$rowff["cf_id"]}' ";
		   ...
		   echo"></center></td>".
		   "<td class=\"tbl-date\"><center>".(!empty($rowff["date_contract_system_created"])?"<input type='button' value='View Contract' onclick=location.href='" . JURI::base() . "contract-listing-vic/contract-folder-vic?quoteid={$rowff['quoteid']}&projectid={$rowff['projectid']}'>":"<input type='button' value='View Costing' onclick=location.href='" . JURI::base() . "view-quote-vic?projectid={$rowff['projectid']}&ref={$ref}'>")."</center>
		   </td></tr>";
