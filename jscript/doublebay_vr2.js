$(document).ready(function(){  
	...
	$('input.num, input.qtylen, input.length, input.width').bind('keypress', function (event) { 
	...
	// var sel_colour = $("input[name='rad_default_color']").val();
	// $("input[name='rad_default_color']").change(function() { 
	...
	if($(".show-form-disable").length>0 || viewType=="read only"){
		...
		$("#project input, #output input,#output select, .table-subtotal-holder input").each(function(){
	...
	$(document).on("change","#output select.desclist, #output select.paint-list",function(e){// alert("trigger");
...
function add_new_louver(){
	...
	$(".added-louvres-len, .added-louvres-qty").change(function(){
			...
			this_pivot_qty = Math.ceil((qty*2)/12);
			
			$(".pivot-strip-qty-"+item_index).val(this_pivot_qty); 
			   
			price = $(".pivot-strip-qty-"+item_index).parent().parent("tr").children("td.td-item").children("input.price").val();
			...
			var rrp = (this_pivot_qty*price);
			$(".pivot-strip-qty-"+item_index).parent().parent("tr").children("td.td-rrp").children("input").val(rrp.toFixed(2));
			...
			//console.log("PIVOT QTY: "+qty);
			$(".link-bar-qty-"+item_index).val(qty); 
			   
			price = $(".link-bar-qty-"+item_index).parent().parent("tr").children("td.td-item").children("input.price").val();
			
			var rrp = (qty*price);
			$(".link-bar-qty-"+item_index).parent().parent("tr").children("td.td-rrp").children("input").val(rrp.toFixed(2));
